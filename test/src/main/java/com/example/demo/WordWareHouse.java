...
public class WordWareHouse {
	...
	public static void main(String[] args) {
		...
		//  SEQUENCE : 오라클의 AutoIncreMent
		//   -> IDENTITY와는 다르게 INSERTY 전에 ID를 알수있긴함.
		//  TABLE : 키 생성 전용 테이블을 하나두고 여기서 ID값을 가져오는 방식
		//   -> 사실 DB 락 유발하는 설정. 현업에서는 잘 사용하지 않음.
		//  AUTO(기본값) : 사용하는 DB에 맞춰서 셋중 하나 JPA가 알아서 선택.
		...
		// @Enumerated
		// Enum 매핑 어노테이션
		//  -> Enum을 쓰는 이유? : 정해진 값들만 들어와야하는 맴버변수(필드)를 관리하기 위해. 
		// EnumType.STRING : Enum의 이름을 DB의 VARCHAR 컬럼에 그대로 저장.
		...
		// @LastModifiedDate : 엔티티의 값이 변경되면 자동으로 시간 저장.
...
//	    // 토심이: "마지막 수정자도 추적 가능합니다!"
		...
		//    - 운영 서버에서는 절대,절대, 절대, 절대, 절대로 금지.
		// 2. create-drop : create와 똑같음 
		//   - 서버가 종료되면 테이블을 모두 삭제. 
		// 3. update : 기존 테이블 구조와 @Entity를 비교해서 바뀐 부분만 반영
		//   - 개발시에는 매우 편리 근데 엔티티 작성하다가 틀리면 불필요한 컬럼이 생길수 있음.
		// 4. validate : 현상 유지 감시 
		//   - 테이블과 엔티티가 일치하는지만 검사 다르면 에러 밷어냄.
		// 5. none
		...
		//  -> 하이버네이트의 물리적 네이밍 전략.
		//  -> 하이버네이트는 기본적으로 데이터베이스는 스네이크케이스로 컬럼을 작성했을거라 생각.
		//  -> 하이버네이트는 그래서 자바에서 카멜케이스로 써두면 db는 거기맞춰 스네이크 케이스
		//     일거라 생각하는 물리적 네이밍 전략을 도입.
		// 에러의 원인 : 즉 하이버네이트가 지 맘대로 스네이크라 생각한거. 
		//  -> 하이버네이트 한테 코드에 있는 그대로 찾으라 명령.
		
		// 테이블간 관계설정
		// 엔티티세계의 관계 설정과 DB세계의 관계 표현방식이 근본적으로 달라
		// 가장 햇갈리는 부분임.
		
		// CascadeType의 종류
		// CascadeType.PERSIST : 부모 엔티티를 저장할때 자식엔티티도 같이 저장.
		// CascadeType.MERGE : 부모 엔티티를 병합할때 자식엔티티도 함께 병합.
		// CascadeType.REMOVE : 부모 엔티티를 삭제할때 자식엔티티도 함께 삭제.
		// CascadeType.REFRESH : 부모 엔티티를 새로고침할때 자식 엔티티도 함께 새로고침
		// CascadeType.DETACH : 부모 엔티티를 분리할때 자식엔티티도 함께 분리
		// CascadeType.ALL : 위의 모든작업을 포함.
